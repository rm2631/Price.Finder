"""
TopDeckBoucherville store scraper implementation.

This module implements the StoreScraper interface for TopDeckBoucherville,
...
class TopDeckBouchervilleScraper(StoreScraper):
    """
    Scraper for TopDeckBoucherville store.

    This scraper searches for Magic: The Gathering cards on TopDeckBoucherville
    and returns available offers including price, set, condition, and availability.
    """

    BASE_URL = "https://www.topdeckboucherville.com"
    SEARCH_URL = f"{BASE_URL}/products/search"
    STORE_NAME = "TopDeckBoucherville"
    ...
    def search(self, card: Card, max_pages: int = 2) -> List[Offer]:
        """
        Search for a card on TopDeckBoucherville and return available offers.
        ...
        """
        try:
            logger.info(f"Searching TopDeckBoucherville for: {card.name}")
            ...
            for page_num in range(1, max_pages + 1):
                ...
                if not page_offers:
                    logger.debug(
                        f"No more results on page {page_num}, stopping pagination"
                    )
                    break
            ...
            if self.use_cache and all_offers:
                save_to_cache(
                    self.STORE_NAME, card.name, self._serialize_offers(all_offers)
                )

            return all_offers

        except requests.exceptions.RequestException as e:
            logger.error(f"Error fetching data from TopDeckBoucherville: {e}")
            return []
        except Exception as e:
            logger.error(f"Unexpected error while searching TopDeckBoucherville: {e}")
            return []
        ...
        try:
            ...
            for product in products:
                try:
                    ...
                    product_url = (
                        self.BASE_URL + url_elem.get("href") if url_elem else ""
                    )
                    ...
                    for variant in variants:
                        try:
                            ...
                            if offer and card_name_matches_query(offer.card, card_name):
                                ...
                                logger.debug(
                                    f"Rejected offer: '{offer.card}' doesn't match query '{card_name}'"
                                )
                        except Exception as e:
    ...
    def _parse_variant(
        ...
        if len(parts) >= 2:
            ...
            language = parts[1]
            # Normalize condition
            condition = self.CONDITION_MAP.get(condition_raw, "Near Mint")
        elif len(parts) == 1:
