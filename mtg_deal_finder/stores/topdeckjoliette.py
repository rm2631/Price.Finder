"""
TopDeckJoliette store scraper implementation.

This module implements the StoreScraper interface for TopDeckJoliette,
...
class TopDeckJolietteScraper(StoreScraper):
    """
    Scraper for TopDeckJoliette store.

    This scraper searches for Magic: The Gathering cards on TopDeckJoliette
    and returns available offers including price, set, condition, and availability.
    """

    BASE_URL = "https://www.topdeckjoliette.com"
    SEARCH_URL = f"{BASE_URL}/products/search"
    STORE_NAME = "TopDeckJoliette"
    ...
    def search(self, card: Card, max_pages: int = 2) -> List[Offer]:
        """
        Search for a card on TopDeckJoliette and return available offers.
        ...
        """
        try:
            logger.info(f"Searching TopDeckJoliette for: {card.name}")
            ...
            logger.error(f"Error fetching data from TopDeckJoliette: {e}")
            return []
        except Exception as e:
            logger.error(f"Unexpected error while searching TopDeckJoliette: {e}")
            return []
