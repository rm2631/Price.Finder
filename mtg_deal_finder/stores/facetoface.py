...
class FaceToFaceScraper(StoreScraper):
    ...
    def search(self, card: Card, max_pages: int = 2) -> List[Offer]:
        ...
        try:
            ...
            for page_num in range(1, max_pages + 1):
                ...
                api_url = (
                    f"{self.API_URL}/keyword/{search_query}/pageSize/50/page/{page_num}"
                )
        ...
        try:
            ...
            for hit in hits:
                ...
                if self._is_non_english(source.get("title", "")):
                    logger.debug(
                        f"Skipping non-English card: {source.get('title', '')}"
                    )
                    continue
    ...
    def _extract_set(self, title: str) -> str:
        ...
        if len(all_brackets) >= 2:
            ...
            if (
                bracket_content.lower() not in ["foil", "non-foil"]
                and not bracket_content.isdigit()
            ):
