"""
MTGJeuxJubes store scraper implementation.

This module implements the StoreScraper interface for MTGJeuxJubes,
...
class MTGJeuxJubesScraper(StoreScraper):
    """
    Scraper for MTGJeuxJubes store.

    This scraper searches for Magic: The Gathering cards on MTGJeuxJubes
    and returns available offers including price, set, condition, and availability.
    """

    BASE_URL = "https://www.mtgjeuxjubes.com"
    SEARCH_URL = f"{BASE_URL}/search"
    STORE_NAME = "MTGJeuxJubes"
    ...
    def search(self, card: Card, max_pages: int = 2) -> List[Offer]:
        """
        Search for a card on MTGJeuxJubes and return available offers.
        ...
        """
        try:
            logger.info(f"Searching MTGJeuxJubes for: {card.name}")
            ...
            for page_num in range(1, max_pages + 1):
                ...
                # MTGJeuxJubes uses a different format: /search?q=...&type=product
                params = {
                    "q": card.name,
                    "type": "product",
                }
            ...
            logger.error(f"Error fetching data from MTGJeuxJubes: {e}")
            return []
        except Exception as e:
            logger.error(f"Unexpected error while searching MTGJeuxJubes: {e}")
            return []
