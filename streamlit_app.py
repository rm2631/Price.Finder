...
def create_excel_download(
    offers: List[Offer], selected_offers: List[Offer] = None
) -> bytes:
    """
    ...
    if not df.empty:
        df = df.sort_values(
            by=["Query", "Selected", "Price"], ascending=[True, False, True]
        )
...
def main():
    ...
    min_quality_choice = st.sidebar.selectbox(
        ...
        index=list(quality_display_names.keys()).index("lp"),
    ...
    with col1:
        search_button = st.button(
            "🔍 Search for Cards", type="primary", use_container_width=True
        )
    with col2:
    ...
    if search_button:
        ...
        try:
            ...
            if total_offers == 0:
                ...
                st.warning(
                    "No offers found for any cards. Try different stores or check your card names."
                )
                return
            ...
            if not selected_offers:
                st.warning(
                    "No suitable offers found matching your criteria. Try relaxing your filters."
                )
                return
            ...
            st.info(
                f"The Excel file will contain all {total_offers} offers with selected offers marked with a ✓"
            )
